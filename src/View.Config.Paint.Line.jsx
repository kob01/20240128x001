...
const settingDefault = { color: '#000000', alpha: 1, width: 2 }
...
const caculateWidth = (path, width, direction) => {
  const min = new Array(Math.floor(width / 2)).fill().map((i, index) => index + 1).map(i => i * 0.5 * -1)
  const max = new Array(Math.floor(width / 2)).fill().map((i, index) => index + 1).map(i => i * 0.5 * 1)
